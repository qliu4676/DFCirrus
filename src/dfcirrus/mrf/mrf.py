"""MRF source-subtraction integration."""
...
def _missing_mrf(exc):
    message = "MRF operations require the external 'mrf' package."
...
def run_wide_subbright(fn_mrf_residual, 
    ...
    # rms
    rms = mad_std(image_mrf, ignore_nan=True)
    print("RMS = %.4g"%rms)
    
    # Mask map
    if mask_data is None:
        mask_data = np.zeros_like(image_mrf, dtype=bool)
...
class MRF_Result:
    ...
    def brightstar_models(self):
        ...
        brightstar_models_shifted = shift(
            np.ma.array(brightstar_models, mask=is_nan),
            [0.25, 0.25],
            cval=0.0,
            order=1,
            mode='nearest',
    ...
    @lazyproperty
    def rms(self):
        return mad_std(self.residual, ignore_nan=True)
